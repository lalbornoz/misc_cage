#!/bin/sh
# $Id$
# Parses the pertinent lines inside /authlog/ produced by sshd(8) for
# successful authentications, printing the corresponding user name,
...
#		syslogd(8) configured logging to /authlog/ as required,
#		ISC BIND's host(1),
#		[mg]awk(1),
...
printf "%-16s  %-14s  %-39s %s\n" "USERNAME" "AUTHENTICATOR" "ADDRESS" "HOSTNAME";
for line in										\
		`awk '/sshd\[[0-9]+\]: Accepted/					\
				{							\
